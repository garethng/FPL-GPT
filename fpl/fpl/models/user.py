...
class User():
    ...
    async def get_gameweek_history(self, gameweek=None):
        ...
        if hasattr(self, "_history"):
            ...
            history = await fetch(
                self._session, API_URLS["user_history"].format(self.id), self.access_token)
    ...
    async def get_picks(self, gameweek=None):
        ...
        if hasattr(self, "_picks"):
            ...
            tasks = [asyncio.ensure_future(
                     fetch(self._session,
                           API_URLS["user_picks"].format(self.id, gameweek), self.access_token))
    ...
    async def get_cup_matches(self, gameweek=None):
        ...
        if not cup:
            cup = await fetch(
                self._session, API_URLS["user_cup"].format(self.id), self.access_token)
            self._cup = cup
    ...
    async def get_team(self):
        ...
        try:
            response = await fetch(
                self._session, API_URLS["user_team"].format(self.id), self.access_token)
    ...
    async def get_transfers(self, gameweek=None):
        ...
        if not transfers:
            transfers = await fetch(
                self._session, API_URLS["user_transfers"].format(self.id), self.access_token)
    ...
    async def get_latest_transfers(self):
        ...
        try:
            transfers = await fetch(
                self._session, API_URLS["user_latest_transfers"].format(self.id), self.access_token)
        except aiohttp.client_exceptions.ClientResponseError:
            raise Exception("User ID does not match provided email address!")
    ...
    async def get_watchlist(self):
        ...
        me = await fetch(self._session, API_URLS["me"], self.access_token)
        return me["watched"]
    ...
    async def transfer(self, players_out, players_in, max_hit=60,
        ...
        players = await fetch(self._session, API_URLS["static"], self.access_token)
        ...
        post_response = await post_transfer(
            self._session, API_URLS["transfers"], json.dumps(payload), self.access_token)
