...
class FPL:
    ...
    async def get_user(self, user_id=None, return_json=False):
        ...
        if return_json:
            return user
        return User(user, session=self.session, access_token=self.access_token)
    ...
    async def get_fixtures(self, return_json=False):
        ...
        :rtype: list
        """
        task = asyncio.ensure_future(fetch(self.session, API_URLS["fixtures"], access_token=self.access_token))
